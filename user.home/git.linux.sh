...
git config --global difftool.p4merge.cmd "/usr/local/bin/p4merge \"\$LOCAL\" \"\$REMOTE\""

echo setup p4merge mergetool
git config --global --unset mergetool.p4merge.cmd
git config --global --unset mergetool.p4merge.path
git config --global mergetool.p4merge.cmd "/usr/local/bin/p4merge \"\$PWD/\$BASE\" \"\$PWD/\$REMOTE\" \"\$PWD/\$LOCAL\" \"\$PWD/\$MERGED\""
